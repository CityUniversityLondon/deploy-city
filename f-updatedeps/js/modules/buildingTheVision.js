...
CITY.buildingTheVision = (function () {
    var
        ...
        latlng = new google.maps.LatLng(51.527361,-0.102365),

        url = "https://www.city.ac.uk/visit/feeds/locations.json?callback=?",
        ...
        processMarkers = function(i, v) {
            if (v.title === "Mechanical/Electrical Infrastructure Project") {
                ...
                if(v.constructionurl) {
                    var latLngBuild = new google.maps.LatLng(parseFloat(v.geoLat), parseFloat(v.geoLong)),
                    ...
                    isBuilding = function (buildingType) {
                        if (buildingType) {
                            infoSettings = {
                                content: "<b>" + v.title + "</b><br />" + description + "<br /><b>Dates:</b> " + v.constructionBegins.split(" ")[0] + " - " + v.constructionFitComplete.split(" ")[0] + " <br /><a href='" + v.constructionurl +"'>More Information...</a>",
                                maxWidth: 300
                            };
                        } else {
                            infoSettings = {
                                content: "<b>" + v.title + "</b><br />" + description,
                                maxWidth: 300
                            };
                        }
                        return infoSettings;
                    ...
                    google.maps.event.addListener(marker, "mousedown", function() {
                        ...
                        infoWindow = new google.maps.InfoWindow(isBuilding(v.projectLocation));
                ...
                $accordion.each(function(l, k) {
                    if ($.trim($(k).text()) === $.trim(v.title)) {
                        $(k).click(function () {
                            ...
                            polygon.setMap(map);
                            map.setZoom(18);
                            map.setCenter(new google.maps.LatLng(v.geoLat, v.geoLong));
                            $freeTextWidget.find(".widget-content .hidden").attr("class", "visible");
        ...
        widgetButtons = function(v, b) {
            $freeTextWidget.find(".removeButton").click( function () {
                b.setMap(null);
                $freeTextWidget.find(".widget-content .visible").attr("class", "hidden");
            });
