    ...
    var // -- VARIABLES
            ...
            init: function(options) {
                ...
                if (hash) {
                    ...
                    if ('function' === typeof settings.initialLoadCallback) {
                        var params = settings.initialLoadCallback.call(
                            this,
                            hash
                        );
                        if (false !== params) {
        ...
        checkDependencies = function() {
            ...
            if (
                !(
                    1 === this.length &&
                    'form' === this.get(0).tagName.toLowerCase() &&
                    'string' === typeof this.attr('id')
                )
            ) {
                $.error(
                    'jQuery.ss must be attached to a single form with an id'
                );
                return false;
            }
        ...
        hijackForm = function() {
            ...
            this.submit(function() {
                ...
                if ('function' === typeof settings.formSerializeCallback) {
                    serialize = settings.formSerializeCallback.call(
                        this,
                        serialize
                    );
                }
            ...
            this.find(
                'input[type=checkbox], input[type=radio], input[type=button], button'
            ).click(function() {
        ...
        hijackPagination = function() {
            $(settings.results).after(
                $('<a/>', {
                    id: settings.nextId,
                    html: 'Load more',
                    href: $(settings.paginationNext).length
                        ? $(settings.paginationNext).attr('href')
                        : 'javascript:;',
                    click: function() {
                    ...
                    style: $(settings.paginationNext).is(':visible')
                        ? null
                        : 'display:none;',
                })
            );
        ...
        processResponse = function(data, textStatus, jqXHR) {
            ...
            if ($(settings.paginationNext, data).length) {
                $('#' + settings.nextId)
                    ...
                    .attr(
                        'href',
                        $(settings.paginationNext, data).attr('href')
                    );
            } else {
                $('#' + settings.nextId).hide();
            }
            if (autoscroll) {
                ...
                $(window).scrollTop(
                    $(settings.results)
                        .find('div.' + settings.pageClass + ':last')
                        .offset().top
                );
            }
        }, // end processResponse
        processInitialLoad = function(params) {
            ...
            $.ajax({
                ...
                error: function(jqXHR, textStatus, errorThrown) {
                    $.error(
                        'AJAX failed with "' + textStatus + '" in jQuery.ss'
                    );
                },
        ...
        processScroll = function() {
            if (
                $('#' + settings.nextId).is(':visible') &&
                $(window).scrollTop() + $(window).height() >=
                    $('#' + settings.nextId).offset().top
            ) {
        ...
        processHashChange = function() {
            ...
            if (0 <= $.inArray(hash, state)) {
                // we've had this state before
                // sit back, relax and...
                return;
            }
