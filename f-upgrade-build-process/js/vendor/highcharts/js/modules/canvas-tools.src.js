    ...
    function build() {
        ...
        svg.Property = function(name, value) {
            ...
            this.Length = {
                ...
                toPixels: function(viewPort) {
                    ...
                    if (s.match(/%$/))
                        return (
                            that.numValue() * svg.ViewPort.ComputeSize(viewPort)
                        );
        ...
        svg.Element.ElementBase = function(node) {
            ...
            this.render = function(ctx) {
                ...
                if (this.attribute('mask').hasValue()) {
                    var mask = this.attribute(
                        'mask'
                    ).Definition.getDefinition();
                    if (mask != null) mask.apply(ctx, this);
                } else if (this.style('filter').hasValue()) {
                    var filter = this.style(
                        'filter'
                    ).Definition.getDefinition();
        ...
        svg.Element.stop = function(node) {
            ...
            if (this.style('stop-opacity').hasValue())
                stopColor = stopColor.Color.addOpacity(
            ...
            this.renderChildren = function(ctx) {
                ...
                if (customFont != null) {
                    ...
                    for (var i = 0; i < text.length; i++) {
                        ...
                        ctx.lineWidth =
                            (ctx.lineWidth * customFont.fontFace.unitsPerEm) /
                            fontSize;
                        if (fontStyle == 'italic')
                            ...
                            if (cssClass == '@font-face') {
                                ...
                                for (var s = 0; s < srcs.length; s++) {
                                    if (srcs[s].indexOf('format("svg")') > 0) {
                                        ...
                                        var fonts = doc.getElementsByTagName(
                                            'font'
                                        );
                                        for (var f = 0; f < fonts.length; f++) {
                                            var font = svg.CreateElement(
                                                fonts[f]
                                            );
                    ...
                    for (var x = 0; x < width; x++) {
                        ...
                        for (var rgba = 0; rgba < 4; rgba++) {
                            var sum =
                                ...
                                (a == 0
                                    ? 255
                                    : imGet(src, x, y, width, height, rgba)) *
                                (a == 0 || rgba == 3 ? 1 : a);
                            for (var i = 1; i < mask.length; i++) {
                                var a1 =
                                    imGet(
            ...
            var draw = function() {
                ...
                if (
                    ...
                    if (e.attribute('width').hasValue())
                        xRatio =
                            e.attribute('width').Length.toPixels('x') /
                            svg.opts['scaleWidth'];
            ...
            svg.intervalID = setInterval(function() {
                ...
                if (svg.opts['ignoreAnimation'] != true) {
                    for (var i = 0; i < svg.Animations.length; i++) {
                        needUpdate =
                            needUpdate |
                            svg.Animations[i].update(1000 / svg.FRAMERATE);
                    }
                }
                ...
                if (
                    svg.opts != null &&
                    typeof svg.opts['forceRedraw'] == 'function'
                ) {
