(function($) {
    var undef,
        ...
        _eventlist =
            'data ready thumbnail loadstart loadfinish image play pause progress fullscreen_enter fullscreen_exit idle_enter idle_exit rescale lightbox_open lightbox_close lightbox_image',
        _events = (function() {
            var evs = [];
            ...
            return {
                ...
                animate: (function() {
                    ...
                    return function(elem, to, options) {
                        options = $.extend(
                            { duration: 400, complete: F, stop: false },
                            options
                        );
                        elem = $(elem);
                        if (!options.duration) {
                ...
                elem: function(elem) {
                    if (elem instanceof $) {
                        return { $: elem, dom: elem[0] };
                    } else {
                        return { $: $(elem), dom: elem };
                    }
                },
                ...
                loadCSS: function(href, id, callback) {
                    ...
                    if ($('#' + id).length) {
                        ...
                        link = $('<link>')
                            .attr({ rel: 'stylesheet', href: href, id: id })
                            .get(0);
                        window.setTimeout(function() {
            ...
            return {
                fade: function(params, complete) {
                    ...
                    Utils.animate(
                        params.next,
                        { opacity: 1 },
                        { duration: params.speed, complete: complete }
                    );
                    if (params.prev) {
                        $(params.prev)
                            .css('opacity', 1)
                ...
                flash: function(params, complete) {
                    $(params.next).css({ opacity: 0, left: 0 });
                    if (params.prev) {
                        Utils.animate(
                            params.prev,
                            { opacity: 0 },
    ...
    Galleria = function() {
        ...
        var divs =
                'container stage images image-nav image-nav-left image-nav-right info info-text info-title info-description thumbnails thumbnails-list thumbnails-container thumb-nav-left thumb-nav-right loader counter tooltip',
            spans = 'current total';
    ...
    Galleria.prototype = {
            ...
            for (i = 0; this._data[i]; i++) {
                ...
                if (o.thumbnails === true && (data.thumb || data.image)) {
                    ...
                    if (
                        ...
                        thumb = { container: null, image: null };
                    }
                }
                $(thumb.container)
                    .add(
            ...
            Utils.wait({
                ...
                success: function() {
                    ...
                    if (/^[0-9]{1,4}$/.test(HASH) && Galleria.History) {
                        ...
                        if (self._data[self._options.show]) {
                            self.show(self._options.show);
                        }
                    }
                },
        ...
        show: function(index, rewind, _history) {
            ...
            this._active = index;
            protoArray.push.call(this._queue, { index: index, rewind: rewind });
    ...
    Galleria.on = function(type, callback) {
        ...
        Galleria.on.binds.push({ type: type, callback: callback || F });
        $.each(Galleria.get(), function(i, instance) {
