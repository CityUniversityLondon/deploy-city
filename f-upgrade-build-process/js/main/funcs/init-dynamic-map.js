...
module.exports = (function() {
    var $ = require('../jquery'),
        onLoad = function(dynamicMap) {
                        ...
                        function() {
                            infoWindow.setContent(
                                '<span class="marker-title">' +
                                    centerMarker.title +
                                    '</span><span class="marker-description">' +
                                    marker.description +
                                    '</span>'
                            );
            ...
            $.ajax({
                url:
                    '//www.city.ac.uk/api/google-map-locations?root=' +
                    locationRootNode,
                dataType: 'json',
            }).done(function(data) {
                if (data !== null) {
                    ...
                    gmap = new google.maps.Map(
                        document.getElementById('dynamic-gmap'),
                        {
                            scrollwheel: false,
                            streetViewControl: false,
                            zoom: gmapData.loadOptions.zoom,
                            center: new google.maps.LatLng(
                                gmapData.loadOptions.latitude,
                                gmapData.loadOptions.longitude
                            ),
    ...
    return function(dynamicMap) {
        if (dynamicMap.length > 0) {
            $.getMultiJsScripts(
                ['lib/google/map/infobox-packed-1.1.9.js'],
                onLoad.bind(null, dynamicMap)
            );
        }
    };
})();
