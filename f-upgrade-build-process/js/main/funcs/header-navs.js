module.exports = function() {
    var $globalNavToggler = $('.global-nav__toggle'),
        ...
        $window = $(window),
        closeMenus = function() {
            $('.header__util__item--active').removeClass(
                'header__util__item--active'
            );
            $('.global-nav__menu-item--active').removeClass(
        ...
        toggleMobileGlobalMenu = function(openMenu) {
            var openSearch =
                openMenu === 'search' &&
                !$headerSearch.hasClass('header__search--active');
            var openNav =
                openMenu === 'nav' &&
                !$globalNavWrapper.hasClass('global-nav__wrapper--active');

            $globalNavWrapper.toggleClass(
        ...
        updateTabNavigation = function() {
            if ($window.width() > $compactWidth) {
                $('.global-nav__menu-item__link').each(function() {
                    if (
                        $(this).siblings('.global-nav__menu-item__toggle')
                            .length
                    ) {
        ...
        updateScrollDown = function() {
            ...
            $body.toggleClass(
                'scroll-down',
                $newScrollTop > $stickyHeaderScrollThreshold &&
        ...
        init = function() {
            ...
            $utilNav.on('click', function(e) {
                var that = $(this),
                    ...
                    targetIsActive = parent.hasClass(
                        'header__util__item--active'
                    );
            ...
            $body.keyup(function(e) {
                if (
                    e.keyCode === 9 &&
                    $(
                        '.header__util__item--active,.global-nav__menu-item--active'
                    ).length > 0 &&
                    !$(e.target).is(
                        '.header__util__item--active,.header__util__item--active *, .global-nav__menu-item--active, .global-nav__menu-item--active *'
                    )
                ) {
                    closeMenus();
                }
