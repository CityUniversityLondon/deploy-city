    ...
    var $menuWrapper = $('.primary-menu-wrapper'),
        ...
        buildMenu = function() {
            $menu
                ...
                .each(function(index) {
                    $ariaLabel =
                        'show or hide items under ' +
                        $(this)
                            .find('.primary-menu__item-text')
                            .text();
                    $(this).append(
                        "<a class='primary-menu__item-toggle' href='#' aria-label='" +
                            $ariaLabel +
                            "'></a>"
                    );
                });
            ...
            $menuToggle.attr(
                'aria-label',
                $menuToggle.find('.primary-menu-toggle__section-name').text() +
                    ' menu'
            );
        },
        highlightCurrentPage = function() {
            var pathname = window.location.pathname
                ...
                .replace('/_recache', '');
            $(
                ".primary-menu .primary-menu__item-link[href$='" +
                    pathname +
                    "']"
            ).addClass('primary-menu__item-link--selected');
        },
        openCurrentSubmenus = function() {
            $menu.find('.primary-menu__submenu').each(function(index) {
                if (
                    $(this).find('.primary-menu__item-link--selected').length >
                    0
                ) {
                    $(this).show();
                    updateToggleLink(
                        $(this)
                            .prev()
                            .children('.primary-menu__item-toggle'),
                        true
                    );
                } else {
                    $(this).hide();
        ...
        openSubmenu = function($toggle) {
            ...
            $toggle
                .parent('.primary-menu__item-interior')
                .next()
        ...
        closeSameLevelSubmenus = function($toggle) {
            var $siblingBranches = $toggle
                .closest('.primary-menu__item--branch')
                .siblings('.primary-menu__item--branch');
            ...
            $.each($siblingBranches, function(index) {
                if (
                    ...
                    updateToggleLink(
                        $(this).find('.primary-menu__item-toggle'),
                        false
                    );
                    $(this)
                        .find('.primary-menu__submenu')
                        .slideUp('medium');
                }
            });
        },
        closeSubmenu = function($toggle) {
            var $enclosingMenuItem = $toggle.closest(
                '.primary-menu__item--branch'
            );

            if (
                $enclosingMenuItem.find('.primary-menu__item-link--selected')
                    .length > 0
            ) {
                ...
                updateToggleLink(
                    $enclosingMenuItem.find('.primary-menu__item-toggle'),
                    false
                );
                $enclosingMenuItem
                    .find('.primary-menu__submenu')
        ...
        initMenu = function() {
            ...
            $menuToggle.attr('aria-expanded', 'false').click(function() {
                ...
                var setOpen = !$menuToggle.hasClass(
