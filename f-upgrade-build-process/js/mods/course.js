...
var onResize = require('./utils/on-resize'),
    ...
    testimonialsBackgroundColor = function() {
        ...
        var selectedItemColor = $(
            'li.course__profiles__item:not([aria-hidden*="true"]) .course__profiles__item__text__title'
        ).css('color');

        // Apply selected text colour to testimonials wrapper background
        $('.course__testimonials__wrapper').css(
            'background',
            selectedItemColor
        );
    },
    ...
    initEmployment = function() {
        ...
        if (wrapper.length) {
            var valuesStr =
                    wrapper.attr('data-values') ||
                    wrapper.attr('data-value') + ';0;0',
                ...
                chart = new google.visualization.PieChart(
                    document.getElementById(
                        'course-destinations-employment-chart'
                    )
                );
    ...
    initCharts = function() {
        charts.onInit(function() {
            onResize(
                composeFunctions(initAssessment(), initDestinations()),
                true
            );
        });
    },
        ...
        if ($(window).width() < 600) {
            $('.course__white-box').each(function() {
                ...
                if (content.height() > 100) {
                    ...
                    var button = $(
                        '<div class="course__white-box__read-more">' +
            ...
            var wrapper = $('.course__kis'),
                update = function() {
                    ...
                    if (currentHorizontal !== neededHorizontal) {
                        var nextSrc = neededHorizontal
                                ? src.replace(/vertical/, 'horizontal')
                                : src.replace(/horizontal/, 'vertical'),
