/*global CITY, $, google */
CITY.widgets = (function(w) {
    'use strict';

    var //local ref to functions from CITY
        debug = CITY.debug,
        createGallery = CITY.createGallery,
        downloadFileTypes = CITY.downloadFileTypes,
        viewportChanged = CITY.viewportChanged,
        /**
        ...
        afterBxSliderLoaded = function(widget, allLink, middleButtonText) {
            var leftWidgetButton = '<i class=" fa fa-caret-left"></i>',
                ...
                inputMiddleButtonText = middleButtonText
                    ? middleButtonText
                    : 'All';
        ...
        initSpotlightResearch = function() {
            var $spotlightResearchWidget = $('#spotlight-research-widget'),
                widgetContent = $spotlightResearchWidget.find(
                    '.widget-content'
                ),
            ...
            if ($('li', spotlightResearchList).length < 2) {
                debug(
                    'Less than 2 spotlight on research, no need for carousel'
                );
                $spotlightResearchWidget
                    .removeClass('widget-loading')
                    .addClass('widget-one-item')
                    .children('.loading-fa-icon')
                    .remove();
        ...
        initCallToAction = function() {
            ...
            if (ed >= cd && sd <= cd) {
                //Show the on date
                $('#content_on_date').attr(
                    'class',
                    'widget-content cta-widget-show'
                );
                $('#content_out_of_date').attr('class', 'cta-widget-hide');
            } else {
                //Show out of date
                $('#content_out_of_date').attr(
                    'class',
                    'widget-content cta-widget-show'
                );
