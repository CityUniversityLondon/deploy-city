        ...
        for (var i = 0; i < pageDropdowns.length; i++) {
            pageDropdowns[i].each(function() {
                ...
                if (
                    ...
                    pageDropdown.get(0) != except.get(0)
                ) {
        ...
        $('.embed-container[data-video-id]')
            ...
            .each(function() {
                ...
                var src =
                    ...
                    videoId +
                    '?rel=0&autoplay=1&enablejsapi=1&wmode=transparent';
...
CITY.tileLayout = (function() {
    ...
    var resultData = function(data) {
        ...
        var md = function(item, key) {
            return item && item.metaData && item.metaData.hasOwnProperty(key)
                ? item.metaData[key]
                : null;
        };
        ...
        return function(data) {
            ...
            for (var i = 0; i < results.length; i++) {
                ...
                if (templates.hasOwnProperty(item.type)) {
                    output += Mustache.render(mainTmpl, item, {
    ...
    var initYouTube = function() {
        if (!isIE7) {
            ...
            var videoPlayed = function(id, iframe, on) {
                ...
                if (inBootstrapMD()) {
                    iframeTile
                        .toggleClass('col-md-24', on)
                        .toggleClass('col-md-12', !on);
                    ...
                    if (on) {
                        ...
                        var iframeTileOrder = iframeTile.attr(
                            'data-tile-order'
                        );
                        for (var i = 0; i < iframeTileOrder; i++) {
                            var selector = '[data-tile-order="' + i + '"]';
                            var isAfter =
                                iframeTile.nextAll(selector).length !== 0;
                            if (isAfter) {
...
CITY.starthere = (function(tileLayout) {
    ...
    var hierarchy = {
        ...
        'School of Health Sciences': [
            ...
            [
                'Speech and Language Communication Sciences',
                'Speech and Language Communication Sciences',
            ],
        ],
        ...
        if (!tileLayout.isIE7) {
            if (localStorage && JSON) {
                var params = JSON.parse(
                    localStorage.getItem('start-here-filter')
                );
            }
            ...
            subject.cityDropdown({
                values:
                    params && params.E in hierarchy
                        ? hierarchy[params.E]
                        : null,
                initialValue:
                    params && params.E in hierarchy && params ? params.F : null,
