CITY.guidelines = (function() {
    ...
    function getStylingNodes(selector) {
        if (typeof queryCache[selector] === 'undefined') {
            queryCache[selector] = Array.prototype.map
                .call(document.querySelectorAll(selector), function(
                    stylesheet
                ) {
    ...
    function getOptions(options) {
        ...
        opts.stylesheets = getStylingNodes(
            opts.stylesSelector || 'link[rel*=stylesheet], style'
        );
    ...
    var $menuToggler = $('.menu-toggler'),
        ...
        guidelinesInit = function() {
            $menuToggler.click(function() {
                ...
                $menuTogglerIcon
                    .toggleClass('icon-reorder')
                    .toggleClass('icon-remove-sign');
            });
            ...
            $('.guidelines-component').each(function() {
                var component = $(this);
                var content = component.find(
                    '.guidelines-component__presentation__contents'
                );

                var templateSrc = component.find('script.hb-template').html();
                var modelDefault = component
                    .find('script.model-default')
                    .html();
                var modelDescription = component
                    .find('script.model-description')
                    .html();
                ...
                if (templateSrc) {
                    ...
                    var codeDisplay = component.find(
                        '.guidelines-component__code'
                    );

                    codeDisplay.html('');

                    codeDisplay
                        .append('<h3>Template</h3>')
                        .append(
                            $('<pre></pre>').append(
                                $('<code></code>').text(templateSrc)
                            )
                        );

                    if (modelDescription) {
                        codeDisplay
                            .append('<h3>Data model</h3>')
                            .append(
                                $('<pre></pre>').append(
                                    $('<code></code>').text(modelDescription)
                                )
                            );
                    }
                    ...
                    if (modelDefault) {
                        codeDisplay
                            .append('<h3>Test data</h3>')
                            .append(
                                $('<pre></pre>').append(
                                    $('<code></code>').text(modelDefault)
                                )
                            );
                    }
                }
            ...
            $("[class*='guidelines-component__controls__']").click(function() {
                ...
                setTimeout(function() {
                    var iframeDocument =
                        ...
                        targetIframe[0].contentWindow.document;
                    targetIframe.attr(
                        'height',
