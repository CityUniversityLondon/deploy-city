$(function() {
    ...
    function initMenu() {
        ...
        $menu
            .find('a')
            .addClass('menu2016__item-link')
            .wrapInner("<span class='menu2016__item-text'></span");
        $menu
            .find('.menu2016__item--branch')
            .children('a')
            .after("<a class='menu2016__item-toggle' href='#'></a>");
    ...
    function highlightCurrentPage() {
        ...
        $(".menu2016 .menu2016__item-link[href$='" + pathname + "']").addClass(
            'menu2016__item-link--selected'
        );
    }
    ...
    function openCurrentSubmenus() {
        $menu.find('.menu2016__submenu').each(function(index) {
            if ($(this).find('.menu2016__item-link--selected').length > 0) {
                ...
                $(this)
                    .prev()
                    .addClass('menu2016__item-toggle--open');
            } else {
                $(this).hide();
                $(this)
                    .prev()
                    .removeClass('menu2016__item-toggle--open');
            }
        });
    }
    ...
    function closeSameLevelSubmenus($toggle) {
        $siblingBranches = $toggle
            .closest('.menu2016__item--branch')
            .siblings('.menu2016__item--branch');

        $.each($siblingBranches, function(index) {
            if ($(this).find('.menu2016__item-link--selected').length <= 0) {
                $(this)
                    .find('.menu2016__item-toggle')
                    .removeClass('menu2016__item-toggle--open');
                $(this)
                    .find('.menu2016__submenu')
                    .slideUp('medium');
            }
        });
    }
    ...
    function closeSubmenu($toggle) {
        ...
        if (
            $enclosingMenuItem.find('.menu2016__item-link--selected').length > 0
        ) {
