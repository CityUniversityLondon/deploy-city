...
CITY.academics = (function() {
    'use strict';

    var // toggler for contact div
        $contactToggler = $('.contact-toggler'),
        // contact info container
        $contact = $('#contact'),
        // results list on index page
        $results = $('#academic-results'),
        // health profiles?
        //isHealth = document.location.pathname.indexOf("health") !== -1,
        // isHealth = false,

        // div containing publication content
        // slightly different place to break pubs up for non health
        $publications = $('#publications .panel-content .accordion ol');

    //dom ready used as init fn
    $(function() {
        $contactToggler.on('click', function(e) {
            e.preventDefault();
            $contact.toggleClass('show');
            $contactToggler.toggleClass('show');
        });

        //init expander on publications
        $publications.expander({
            slicePoint: 2000,
            preserveWords: true,
            expandPrefix: '',
            expandEffect: 'show',
        ...
        if ($results.length) {
            // init index page here

            yepnope([
                'lib/backbone/backbone-build.min.js',
                'modules/profiles-search/academics-backbone.js',
            ]);
        } // end if on index page
    }); //end DOM ready
})(); //end CITY.academics
/* jshint ignore:start */
/*!
        ...
        function z(a, d) {
            d.stop(true, true)[a.collapseEffect](a.collapseSpeed, function() {
                d.prev('span.' + a.moreClass).show().length ||
                    d
                        .parent()
                        .children('div.' + a.summaryClass)
                        .show()
                        .find('span.' + a.moreClass)
                        .show();
                ...
                this.each(function() {
                    ...
                    if (!c.data(this, 'expanderInit')) {
                        ...
                        w.find('a')
                            ...
                            .bind('click.expander', function(j) {
                                j.preventDefault();
                                w.hide();
                                F.hide();
