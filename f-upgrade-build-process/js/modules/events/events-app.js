        ...
        eventN = function() {
            var /**
                ...
                initBookingForm = function() {
                    ...
                    if (
                        $bookingForm.find('.wFormThankYou').length ||
                        $bookingForm.find('.errorMessage').length
                    ) {
                        showForm();
                    }
                    ...
                    ajaxFormOptions = {
                        ...
                        success: function(responseText, statusText) {
                            ...
                            CITY.debug(
                                'form submission resulted in: ' + statusText
                            );
        ...
        eventsCalendar = function(B, M) {
             ...
             */
                CalendarCollection = Backbone.Collection.extend({
                    ...
                    url: function() {
                        var url =
                            '/fb/search.json?collection=city-events&meta_P_orsand=%22Yes%22&landing=true&sort=adate&_=1373022367687';
                        var dept = $('div.event-calendar-wrap').data(
                            'department'
                        );
                        var category = $('div.event-calendar-wrap').data(
                            'category'
                        );
                        ...
                        if (window.location.search) {
                            var schoolMatch = window.location.search.match(
                                /[\?&](meta_E_orsand=[^&]+)(&|$)/
                            );
                            if (schoolMatch) {
                                url += '&' + schoolMatch[1];
                    ...
                    parse: function(response) {
                        ...
                        if (
                            !response.response ||
                            !response.response.resultPacket
                        ) {
                            return retval;
                        }
                        // loop over each result and flatten (only 1 level at this point) then push into retval
                        _.each(response.response.resultPacket.results, function(
                            result
                        ) {
                ...
                dateToFbDate = function(date) {
                    ...
                    return {
                        ...
                        dyear: this.dyear,
                        dateString:
                            this.dday + '-' + this.dmonth + '-' + this.dyear,
                    };
                },
                /**
                ...
                LinksView = M.ItemView.extend({
                    ...
                    template: function() {
                        ...
                        return _.template(linksTemplate, this, {
                            variable: 'attrs',
                        });
                    },
                }),
                ...
                CalendarView = Backbone.View.extend({
                    ...
                    calendarBuild: function(calendarObj) {
                        ...
                        this.fbUrl =
                            '/fb/search.json?collection=city-events&sort=adate&meta_P_orsand=%22Yes%22&form=calendar&date=all&landing=true';
                        this.noOfDaysFromNow = new Date();
                        this.today = new Date();
                        this.endOfMonthObj = new Date(
                            this.today.getFullYear(),
                        ...
                        this.fbEndOfMonth = dateToFbDate(this.endOfMonthObj);
                        this.fbToday = dateToFbDate(this.today);
                        this.noOfDaysFromNow.setDate(
                            this.noOfDaysFromNow.getDate() + 365
                        );
             ...
             */
                EventsCollection = Backbone.Paginator.clientPager.extend({
                    ...
                    searchFunnelback: function(query) {
                        ...
                        var data = decodeURIComponent(
                                $.param(
                                    _.extend(this.server_api, { query: query })
                                )
                            ),
                            self = this;
                ...
                EventItemView = M.ItemView.extend({
                    ...
                    template: function(modelData) {
                        //l(modelData);
                        return _.template(eventTemplate, modelData, {
                            variable: 'data',
                        });
                    } /* not used ....yet? ,
                ...
                SearchMetaView = M.ItemView.extend({
                    ...
                    template: function(modelData) {
                        modelData.info =
                            APP.collections.eventsCollection.information;
                ...
                MainRouter = B.Router.extend({
                    ...
                    initialize: function() {
                        ...
                        // and DOM is ready
                        APP.collections.eventsCollection.once(
                            'sync',
                            function() {
                                l('starting router');
                                $(function() {
