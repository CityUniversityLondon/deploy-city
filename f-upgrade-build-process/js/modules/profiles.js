CITY.profiles = (function() {
    var fbCollection = 'profiles',
        ...
        $fbProfilesWwrapperResultsOl = $(
            '#fb-profiles-wrapper #results-wrapper ol'
        ),
        hashString,
        hashStringDeparam,
        ...
        removeFilter = function() {
            ...
            $contentWrapper.on('click', '.display-filters a', function(e) {
                $.bbq.removeState(
                    $(this)
                        .parent('li')
                        .attr('data-filter')
            ...
            if (hashString === '' && event === undefined) {
                ...
                $.ajax({
                    url:
                        ...
                        '&meta_B_orsand=' +
                        familyType +
                        '&simpleResults=true&sort=metaK&bustCache=' +
                        Math.random(),
                            ...
                            if (key === 'meta_C_orsand') {
                                ...
                                $.each(metaCList, function(index, value) {
                                    $subfamilyChoice
                                        ...
                                        .addClass('active')
                                        .append(
                                            '<span class="remove-filter"><i class="fa fa-times"></i><span class="visuallyhidden">remove filter</span></span>'
                                        );
                                    ...
                                    $fbProfilesWwrapperResultsOl
                                        .find(
                                            '.subfamily a[data-filter="meta_C_orsand=[\'' +
                                                value +
                                                '\']"]'
                                        )
                                        .addClass('active')
                                ...
                                $displayFilters.append(
                                    '<li data-filter=' +
                                        key +
                                        '><a href="#">' +
                                        value.replace(/"/g, '') +
                                        '<span class="remove-filter"><i class="fa fa-times"></i><span class="visuallyhidden">remove filter</span></span></a></li>'
                                );
                                //highlight the selected filters everywhere on the page
                                $fbProfilesWwrapperResultsOl
                                    .find(
                                        '.tag-list a[data-filter="' +
                                            key +
                                            '=' +
                                            value +
                                            '"]'
                    ...
                    if (el.parents('ul:eq(0)').hasClass('tag-list')) {
                        ...
                        if (el.hasClass('active')) {
                            if (el.parent('li').hasClass('subfamily')) {
                                ...
                                queryStringArray = metaChash.meta_C_orsand
                                    .replace('[', '')
                                    .replace(']', '')
                                    .split(',');
                                queryStringArray
                                    ...
                                    .join(',');
                                newMetaCParam =
                                    'meta_C_orsand=[' + queryStringArray + ']';
                        ...
                        //add the clicked filter to the hash, but do not touch the other filters
                            ...
                            if (
                                metaChash.meta_C_orsand === undefined ||
                                metaChash.meta_C_orsand
                                    .replace('[', '')
                                    .replace(']', '')
                                    .split(',')[0] === ''
                            ) {
