    ...
    var init = function() {
            var pageBody = $('html, body'),
                ...
                noResults = $('<li/>', {
                    ...
                    html:
                        '<p>Sorry, your search has resulted in 0 matches. Try selecting fewer filters or <a href="" id="reload">Start Again<span>Refresh Page</span></a>',
                }),
                formsCallback,
            ...
            function checkFilters() {
                indexForm
                    .find(':radio:checked')
                    .add(':checkbox:checked', indexForm)
                    .not('#all-dates')
                    .not('.hidden')
                    .each(function() {
                        var self = $(this),
                        ...
                        activeFilter = $('<li class="active-filter"></li>')
                            .html(
                                '<a href="#" title="remove this filter">' +
                                    label +
                                    '<span class="close">remove</span></a>'
                            )
                            .click(function(e) {
            ...
            function eventsResponse(responseText) {
                ...
                responseText.find('result').each(function() {
                    ...
                    resultsItems +=
                        '<li class="vevent event"><div class="date"><p class="date-month"><abbr class="dtstart" title="' +
                        iCal +
                        '">' +
                        month +
                        ' ' +
                        year +
                        '</abbr></p><p class="date-day-no">' +
                        dayNo +
                        '</p><p class="date-day">' +
                        day +
                        '</p>' +
                        duration +
                        '</div><div class="event-details"><h2><a href="' +
                        url +
                        '" title="' +
                        title +
                        '" class="summary">' +
                        ...
                        '</a></h2><p class="time">' +
                        time +
                        '</p><p class="category">Category: ' +
                        category +
                        '</p><p class="audience">Open to: ' +
                        audience +
                        '</p><p class="event-summary">' +
                        summary +
                        '</p></div></li>';
                }); //format results
            ...
            indexForm
                ...
                .hide()
                .after($('<a href="#" class="reveal">Show All...</a>'));

            $('.reveal').live('click', function(e) {
                var clicked = $(e.target),
                    previous = $(e.target).prev();
