...
CITY.youtubePreview = (function () {
    var init = function (callback) {
        $('.embed-container[data-video-id]').not('[data-video-id-ready]').each(function () {
            var container = $(this);
            var imgGroup = container.find('.youtube-preview');
            container.attr('data-video-id-ready', 1);

            var videoId = container.attr('data-video-id');
            var src = 'https://www.youtube.com/embed/' + videoId + '?rel=0&autoplay=1&enablejsapi=1&wmode=transparent';
            var iframeId = container.attr('data-video-iframe-id');

            imgGroup.click(function () {
                $('<iframe></iframe>').attr('id', iframeId).attr('src', src).attr('frameborder', 0).attr('allowfullscreen', 1).appendTo(container);
                if (callback) {
                    callback();
                }
            });
...
CITY.tileLayout = (function () {
    ...
    var processResults = (function () {
        var mainTmpl = '<div class="col-md-12 tile tile-type-{{type}} {{^live}}tile-preview{{/live}}" data-tile-id="{{id}}"><div class="tile-canvas"><div class="tile-content">{{> content}}</div></div></div>';
        ...
        $("[id^='tmpl-tile-type-']").each(function () {
            var t = $(this).html();
            Mustache.parse(t);
            templates[$(this).attr('id').substr(15)] = t;
        });
    ...
    var query = function (nextPage) {
        if (!busy) {
            ...
            var url = 'http://www.city.ac.uk/fb/search.json';
